...
def _projection_metrics(start, end, p) -> Tuple[float, float]:
...
def find_waypoints_along_direction(start, end, max_n=3, corridor_km=30.0,
                                   max_abs_detour_km=50.0, max_rel_detour=0.35):
    if not beach_coords: return []
    cands = []
    for name, (lon, lat) in beach_coords.items():
        t, off = _projection_metrics(start, end, (lat, lon))
        if 0.0 < t < 1.0 and off <= corridor_km:
...
# ---------------- TourAPI (안정 호출: 키/프로토콜 조합 + 반경 단계 상승) ----------------
...
def _tourapi_request(path: str, params_wo_key: Dict[str, Any], timeout=10):
    ...
    for scheme in ("https", "http"):  # 환경 따라 한쪽만 응답하는 경우가 있음
        for key_type, key_val in _key_variants():
            ...
            try:
                ...
                j = None; result_code=None; result_msg=None; items_cnt=None
                if status == 200:
                    try:
                        j = r.json()
...
def _normalize(item: Dict[str,Any], intro: Dict[str,Any], common: Dict[str,Any],
               category: str, src_lat: float, src_lon: float, ctype: int):
...
def _sample_idxs(coords: List[List[float]], interval_km: float, max_samples=100):
...
def search_tour_items_along_route(geojson: Dict[str,Any], corridor_km=30.0, limit_each=60):
    try:
        coords = geojson["features"][0]["geometry"]["coordinates"]
    except Exception:
        return {"tour":[], "food":[], "all":[]}

    radii = [5000, 8000, 12000, 20000]  # 5→8→12→20km
    interval_km = 10.0 if corridor_km>=25.0 else max(7.0, corridor_km*0.35)
    idxs = _sample_idxs(coords, interval_km, max_samples=100)
    seen=set(); tours=[]; foods=[]

    def collect(ctype: int, bucket: list):
        for r in radii:
            if len(bucket) >= limit_each: break
            for i in idxs:
                if len(bucket) >= limit_each: break
                lon, lat = coords[i]
                items, _ = _location_based(lon, lat, ctype=ctype, radius_m=r, rows=40)
                for it in items:
...
def _handle_route():
    ...
    resp = {
        ...
        "spots": spots["all"],  # 프런트가 사용하는 기본 목록
...
def debug_tourapi_near():
    ...
    radii=[5000,8000,12000,20000]; tries_total=[]; items=[]
    for r in radii:
        its, meta = _location_based(lon, lat, ctype=ctype, radius_m=r, rows=40)
        tries_total.extend(meta.get("tries", []))
        if its: items = its; break
    sample=[{"contentid":it.get("contentid"),"title":it.get("title"),"addr1":it.get("addr1"),
             "mapx":it.get("mapx"),"mapy":it.get("mapy"),"firstimage":it.get("firstimage")} for it in items[:5]]
    return jsonify({"ok":True,"lat":lat,"lon":lon,"ctype":ctype,"tries":tries_total,"count":len(items),"sample":sample})
