...
def filter_waypoint_candidates(start, end):
    ...
    # 해안 범위 (대한민국 기준)
    coast_filtered = road_points[
        ((road_points["x"] >= 124) & (road_points["x"] <= 131)) &
        ((road_points["y"] >= 33) & (road_points["y"] <= 38))
    ]
    ...
    # 최종 후보 필터링
    candidates = coast_filtered[
        (coast_filtered["dist_to_start"] <= 3) & coast_filtered.apply(direction_filter, axis=1)
    ]
    ...
    print("✅ 선택된 waypoint 후보:", selected["y"], selected["x"])
...
def route():
    try:
        ...
        waypoint = filter_waypoint_candidates(start, end)
        if not waypoint:
