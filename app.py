...
# TourAPI는 KorService2를 사용 (국문 GW 승인키)
TOURAPI_KEY = os.getenv("TOURAPI_KEY")  # 포털의 Encoding 키(퍼센트 인코딩된 값) 권장
TOURAPI_BASE = os.getenv("TOURAPI_BASE") or "https://apis.data.go.kr/B551011/KorService2"

# =========================================================
# index.html 서빙 (없으면 임시 페이지)
# =========================================================
...
def _fallback_index_html() -> str:
    return """<!DOCTYPE html><html lang='ko'><head><meta charset='UTF-8'><title>Coastal Drive</title></head><body>
    <h3>index.html이 없습니다.</h3><p>동작 확인: <a href="/healthz">/healthz</a></p></body></html>"""
...
# =========================================================
# TourAPI KorService2 (locationBasedList1 + detailIntro1)
# =========================================================
def _tourapi_get(path: str, params: Dict[str, Any], timeout=8) -> Tuple[Dict[str, Any], int, str]:
    """TourAPI 호출(응답 JSON/상태/resultCode 추출)."""
    if not TOURAPI_KEY:
    ...
    try:
        url = f"{TOURAPI_BASE.rstrip('/')}/{path.lstrip('/')}"
        r = requests.get(url, params=p, timeout=timeout)
        try:
            j = r.json()
        except Exception:
            # JSON 아님(HTML 등) → 텍스트만
            return {"error": "Non-JSON response", "text": r.text[:500]}, r.status_code, ""
        result_code = (j.get("response") or {}).get("header", {}).get("resultCode", "")
        return j, r.status_code, result_code
    except Exception as e:
        return {"error": str(e)}, 500, ""

@lru_cache(maxsize=4096)
def _tour_location_based(lon: float, lat: float, content_type_id: int, radius_m: int, num_rows: int) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    j, status, rc = _tourapi_get(
        "locationBasedList1",
        params={
            "mapX": lon, "mapY": lat,
            "radius": radius_m,
    ...
    debug = {"status": status, "resultCode": rc, "radius_m": radius_m}
    if status != 200 or "error" in j:
        return [], debug
...
def _normalize_detail(item: Dict[str, Any], intro: Dict[str, Any], category: str) -> Optional[Dict[str, Any]]:
...
def search_tour_items_along_route(geojson: Dict[str, Any], limit_each: int = 25) -> Tuple[Dict[str, List[Dict[str, Any]]], Dict[str, List[Dict[str, Any]]]]:
    """경로 주변(샘플링)에서 관광(12), 맛집(39) 수집. KorService2 사용. 디버그 포함."""
    try:
        coords = geojson["features"][0]["geometry"]["coordinates"]
    except Exception:
        return {"tour": [], "food": [], "all": []}, {"tour": [], "food": []}
    ...
    def collect(radius_m: int, sample_goal: int):
        ...
        for idx in range(0, len(coords), step):
            lon, lat = coords[idx]
            # 관광(12)
            items, dbg = _tour_location_based(lon, lat, 12, radius_m, 12)
            debug_t.append(dbg)
            for it in items:
        ...
        for idx in range(0, len(coords), step):
            lon, lat = coords[idx]
            items, dbg = _tour_location_based(lon, lat, 39, radius_m, 12)
            debug_f.append(dbg)
            for it in items:
                cid = str(it.get("contentid") or "")
                if not cid or cid in seen:
            ...
            if len(foods) >= limit_each:
                break
        return tours, foods, debug_t, debug_f

    # 1차 5km, 부족하면 8km 리트라이
    t1, f1, dt1, df1 = collect(radius_m=5000, sample_goal=200)
    if len(t1) + len(f1) < 10:
        t2, f2, dt2, df2 = collect(radius_m=8000, sample_goal=300)
        if len(t2) > len(t1): t1, dt1 = t2, dt2
        if len(f2) > len(f1): f1, df1 = f2, df2

    all_items = t1 + f1
    debug = {"tour": dt1, "food": df1}
    return {"tour": t1, "food": f1, "all": all_items}, debug

# =========================================================
# 라우팅 핸들러 (경로 로직 그대로, TourAPI 실패해도 JSON 보장)
# =========================================================
def _handle_route():
    try:
        data = _coerce_json()
        ...
        # TourAPI는 실패해도 경로는 내려가게 try/except
        spots, tourapi_debug = {"tour": [], "food": [], "all": []}, {"tour": [], "food": []}
        try:
            spots, tourapi_debug = search_tour_items_along_route(route_data)
        except Exception as e:
            log.warning("TourAPI search failed: %s", e)
        ...
        # 디버그를 보고 싶으면 /route?debug=1 로 호출
        if str(request.args.get("debug") or data.get("debug") or "") == "1":
            resp["tourapi_debug"] = tourapi_debug

        return jsonify(resp), 200

    except Exception as e:
        # 어떤 경우에도 JSON 보장
        log.exception("route handler crashed")
...
# TourAPI 핑/디버그 (브라우저에서 바로 확인)
@app.route("/debug/tourapi_ping")
def tourapi_ping():
    lon = _to_float(request.args.get("lon") or "127.5")
    lat = _to_float(request.args.get("lat") or "36.5")
    if lon is None or lat is None:
        return jsonify({"error": "lon/lat 필요"}), 400
    items, dbg = _tour_location_based(lon, lat, 12, 5000, 10)
    return jsonify({"count": len(items), "debug": dbg, "sample": items[:2]}), 200
