...
def find_best_beach_waypoint(start, end):
    """
    '전에 성공하던' 방식:
    - 출발과 위도(±0.2) 가까운 후보 중 진행방향 같은 쪽
    - 출발과 경도(±0.2) 가까운 후보 중 진행방향 같은 쪽
    - 각 집합에서 '도착과의 거리'가 가장 짧은 것 비교 선택
    """
...
def find_second_beach_waypoint_simple(first_wp):
    """
    경유2는 '경유1과 가장 가까운 해수욕장'으로 단순 선택(동일 좌표 제외).
    """
